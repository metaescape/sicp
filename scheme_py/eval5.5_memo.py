...
use memoization to speed up the evaluation
...
def evaluate(exp: list, env: list = []):
    if isinstance(exp, (int, float)):
        return exp
    if type(exp) is str:
        value, frame = lookup(exp, env)
        if type(value) is tuple:
            e, env, actual_v = value
            if actual_v is None:
                actual_v = evaluate(e, env)  # recursion here to expand delay
                frame[exp] = (e, env, actual_v)
            return actual_v
...
def apply(proc, args):
    ...
    return proc([evaluate(*arg[:2]) for arg in args])
